"""
Web Search Module for Climate Risk Assessment - ENHANCED VERSION
Uses DuckDuckGo with comprehensive queries and strict physical climate risk focus
"""
import logging
...
def search_company_climate_info(company_name: str, max_results: int = 25) -> List[Dict]:
    """
    Search for company-specific PHYSICAL climate risk information
    
    ENHANCED VERSION with comprehensive queries targeting physical climate risks
    ...
    """
    logger.info(f"Searching for PHYSICAL climate risk info: {company_name}")
    
    # ENHANCED: 8 targeted queries focusing on physical climate risks
    queries = [
        # Physical risk specific
        f'"{company_name}" physical climate risk assessment',
        f'"{company_name}" climate adaptation strategy resilience',
        f'"{company_name}" extreme weather vulnerability',
        
        # Report-specific (likely to contain detailed disclosures)
        f'"{company_name}" sustainability report physical risks',
        f'"{company_name}" TCFD physical risk disclosure',
        f'"{company_name}" CDP climate change response',
        
        # Regulatory filings (comprehensive disclosures)
        f'"{company_name}" 10-K climate physical risks',
        f'"{company_name}" ESG report climate hazards adaptation'
    ]
    ...
    try:
        with DDGS() as ddgs:
            for query in queries:
                ...
                try:
                    # Get 4 results per query (8 queries × 4 = 32 potential results)
                    ...
                    for result in results:
                        ...
                        if is_company_specific(result, company_name):
                            ...
                            seen_urls.add(url)
                            logger.debug(f"✓ Added: {result.get('title', '')[:60]}")
                        else:
                            logger.debug(f"✗ Filtered: {result.get('title', '')[:60]}")
        ...
        logger.info(f"Found {len(all_results)} company-specific PHYSICAL climate risk results for {company_name}")
...
def format_search_results(results: List[Dict]) -> str:
    ...
    formatted = "## Web Search Results (Company-Specific Physical Climate Risk Information)\n\n"
